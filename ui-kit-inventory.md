...
| dropdown-list — A-basic | _POC-original (not in kit)_ | molecule (Alpine) | `1343:42718` list, `1343:42177` list item | approved (2026-04-24) | Phase 1 complete. Floating menu panel with Alpine combobox pattern: `aria-expanded` / `aria-controls`, keyboard nav (arrows / Enter / Escape), option selection. First Alpine-based molecule in the POC. Will replace native `<select>` in form-field variants later (country selector, phone prefix, size selector). |
| form-checkbox — A-basic | _POC-original (not in kit)_ | atom (CSS) | `1420:30806` | approved (2026-05-08) | Phase 1 complete. Native `<input type="checkbox">` styled via `appearance: none` + custom check glyph. Single `@utility form-checkbox` shell on wrapping `<label>` driven by `:has()` state selectors and CSS custom properties — mirrors form-field architecture. Variants: unchecked / checked / indeterminate. States: hover / focus-visible / disabled / invalid. Sizes: S / M / L. Feedback modifiers: error / warning / success. Group wrapper: `form-checkbox-group` (`<fieldset>` + `<legend>`). CSS-only — indeterminate seeded via one-liner module script. |
...
| swatches — A-swatches-rounded | `hyva-ui-reference/components/swatches/A-swatches-rounded/` | product (CSS atom) | `1385:32405` | approved (2026-05-08) | Phase 1 complete. CSS-only attribute-picker / facet atom. ONE `@utility swatch` on a wrapping `<label>` drives the visible chip via CSS custom properties; state derives from `:has(:checked)` / `:has(:focus-visible)` / `:has(:disabled)` / `.is-out-of-stock` / `[aria-disabled="true"]`. Three types (color / text / image), two shapes (round default + rectangle), five sizes (XS / S / M / L / XL — 2XL/3XL deferred). Selected color/image uses the full `Additional/Swatch inner` shadow + 2 px outer ring; text-swatch selection shifts label to emerald-500 + weight 600. Disabled / out-of-stock = `linear-gradient` diagonal slash + 0.6 opacity (identical visual, semantic difference only). Single inline-style whitelist: `style="--swatch-bg: <hex>"` for consumer-supplied product hex. No Alpine. |
